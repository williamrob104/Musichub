...
class StringAlgorithmTest {
    @Test fun testLongestCommonSubsequence() {
        ...
        for ((str1, str2, lcs) in testCases)
            assertEquals("longestCommonSubsequence($str1, $str2) != $lcs",
                longestCommonSubsequence(str1, str2, equals), lcs)
    }

    @Test fun testLongestCommonSubstring() {
        val equals = { i1: Int, i2: Int -> i1 == i2 }
        val testCases = listOf(
                Triple("abc", "abc", 3),
                Triple("abc", "abcde", 3),
        ...
        for ((str1, str2, lcs) in testCases)
            assertEquals("longestCommonSubstring($str1, $str2) != $lcs",
                longestCommonSubstring(str1, str2, equals), lcs)
    }

    @Test fun testEditDistance() {
        val equals = { i1: Int, i2: Int -> i1 == i2 }
        val testCases = listOf(
                Triple("abc", "abc", 0),
                Triple("abc", "abcde", 2),
        ...
        for ((str1, str2, ed) in testCases)
            assertEquals("editDistance($str1, $str2) != $ed",
