...
internal inline fun longestCommonSubsequence(str1: String, str2: String, matchFunc: (Int, Int) -> Boolean=::sameEnglishLetter): Int {
...
internal inline fun longestCommonSubstring(str1: String, str2: String, matchFunc: (Int, Int) -> Boolean=::sameEnglishLetter): Int {
    val seq1 = str1.codePointsCompat()
    val seq2 = str2.codePointsCompat()
    val s1 = if (seq1.size < seq2.size) seq2 else seq1
    val s2 = if (seq1.size < seq2.size) seq1 else seq2
...
internal inline fun editDistance(str1: String, str2: String, matchFunc: (Int, Int) -> Boolean=::sameEnglishLetter,
                                 insertOrDeleteCost: Int=1, substituteCost: Int=1): Int {
    val seq1 = str1.codePointsCompat()
    val seq2 = str2.codePointsCompat()
    val s1 = if (seq1.size < seq2.size) seq2 else seq1
    val s2 = if (seq1.size < seq2.size) seq1 else seq2
        ...
        for (j in 1..s2.size) {
            if (matchFunc(s1[i - 1], s2[j - 1])) {
                ...
                dyn[curr][j] = arrayOf(
                    ...
                    dyn[curr][j-1] + insertOrDeleteCost).min()!!
            }
        }
    }
    return dyn[s1.size % 2][s2.size]
